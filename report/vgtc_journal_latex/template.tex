...
%% Paper title.
\title{Characterising Responses to Events through a Spatiotemporal Analysis of Tweets}
...
Tweets and related twitter data are a particularly useful dataset in the realm of spatiotemporal data. This is because they are mostly submitted by individual users, and are often localised in both space and time. Because of this, twitter data has often been used by authors to identify and track unusual occurrences such as dramatic news events usually through a keyword analysis [1], or used twitter data to discover otherwise unreported events [2]. It would be interesting, however, to investigate whether twitter data can be effectively used to discover and characterise otherwise widely observed and regular events. Such an approach may likely make it possible, with sufficient data, to characterise spatial regions in aggregate, and may provide a foundation both to identify underlying similarities between regions and to predict future behaviours.

\subsection{Data}

The dataset used in this project comes set of almost 120,000 tweets located primarily in the Greater London area, over a period of 48 hours between 4am 31st December 2014 and 4am 2nd January 2015. Importantly, this period of time covers a set of widely observed, time-localised, events such as New Year’s Eve. Altogether, the dataset contains the following features for each tweet,

\begin{itemize}
\item Longitude and latitude\
\item Message date\
\item Username and id\
\item Message test\
\item Location\
\item Hashtags\
\item Flag to indicate whether it was a retweet (although no tweets in the raw dataset are retweets)\
\end{itemize}

Tweets contained in the dataset also include a range of messages between users and shared media. The vast majority of tweets are in English and are person generated.
It is important to note that this set of twitter data was provided by City University for the purpose of this project, so I cannot comment on how it was originally obtained.


\subsection{Research Questions}

Overall this project aims to see if it’s possible not only to identify major events from twitter data, but whether geographical areas and time intervals can be characterised by their twitter usage. Breaking the project down into specific questions,

\begin{itemize}
\item How does twitter activity vary across both space and time for this dataset?\
\item Can boroughs of London be grouped by their twitter activity and how?\
\item Can specific events be identified through twitter usage and how?\
\item How are events reported through twitter data?\
\end{itemize}
